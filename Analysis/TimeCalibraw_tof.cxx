...
string exeversion2="---------TimeCalib[A1][T1]->Fill(-Tdiff-(peakV[A1][T1]) );---- ";
string exeversion3="---------TimeCalib[A2][T2]->Fill( Tdiff-(peakV[A2][T2]) ); ----- ";
string exeversion4="----------all  mean adn entries in hist diff -------------  ";
...
//TApplication *a = new TApplication("a", 0, 0);
      ...
      double histmean,histsigma;


	  TH1F *TimeCalib[NZX][NCX];
	  TH2F *TimevsS[NZX][NCX];
	  TH2F *TimevsTheta[NZX][NCX];
	  for(int i=0 ;i<NZX;i++){
		  for(int j=0 ;j<NCX;j++){
			  sprintf(buf,"TimeCalib_%d_%d",i,j);
			  TimeCalib[i][j] = hist1d(buf,100,-5,5,"TimeDiff","Entries");
			  sprintf(buf,"TimevsS_%d_%d",i,j);
			  TimevsS[i][j] = hist2d(buf,1000,-500,500,100,-5,5,"S (mm)","Time diff(ns)");
			  sprintf(buf,"TimevsTheta_%d_%d",i,j);
			  TimevsTheta[i][j] = hist2d(buf,1000,-500,500,100,-5,5,"Theta (rad)","Time diff(ns)");
		  }
	  }
      TH1D *TIMERES=hist1dD("TIMERES",100,-5,5,"TimeDiff","Entries");
      TH2F *TimeResvsS=hist2d("TimeresvsS",1e3,-500,500,100,-5,5,"S (mm)","Time diff(ns)");
...
for(int FF=0;FF<Niter;FF++){
	...
	//for(int KK=0;KK<1e6;KK++){
		...
		TimeCalib[A1][T1]->Fill(-Tdiff-(peakV[A1][T1]) );
		TimeCalib[A2][T2]->Fill(Tdiff-(peakV[A2][T2]) );
		
		TimevsS[A1][T1]->Fill(S,-Tdiff-(peakV[A1][T1]) );
		TimevsS[A2][T2]->Fill(S, Tdiff-(peakV[A2][T2]) );
		TimevsTheta[A1][T1]->Fill(theta,-Tdiff-(peakV[A1][T1]) );
		TimevsTheta[A2][T2]->Fill(theta, Tdiff-(peakV[A2][T2]) );
		
		TIMERES->Fill(-Tdiff-(peakV[A1][T1]) );
		TIMERES->Fill(Tdiff-(peakV[A2][T2]) );
		TimeResvsS->Fill(S,Tdiff-(peakV[A2][T2]) );
		TimeResvsS->Fill(S,Tdiff-(peakV[A1][T1]) );
	}
...
//////****************************************************************//;	
	...
	sprintf(buf,"%s/TimeCalib_ToF_V1_Iter%d_rawroot.root",filepath.c_str(),FF);
	
	file=new TFile(buf,"recreate");
	tr =new TTree("tr","mean");
	tr->Branch("Ax",&Ax,"Ax/I");
	tr->Branch("Tx",&Tx,"Tx/I");
	tr->Branch("offset",&offset,"offset/D");
	tr->Branch("histmean",&histmean,"histmean/D");
	tr->Branch("histsigma",&histsigma,"histsigma/D");
	file->cd();
	for(int i=0;i<NZX;i++){
		for(int j=0; j<NCX;j++){
			TimeCalib[i][j]->Write();
			TimevsS[i][j]->Write();
			TimevsTheta[i][j]->Write();
		    Ax =i;
			Tx = j;
			offset = peakV[i][j];
		    histmean = TimeCalib[i][j]->GetMean();	
		    histsigma= TimeCalib[i][j]->GetRMS();	
			tr->Fill();
		}
	}
	tr->Write();
