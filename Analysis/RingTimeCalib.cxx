...
string exeversion4="----------Two PEAKS in the histgram-------------  ";
...
//TApplication *a = new TApplication("a", 0, 0);
	  ...
	  for(int i=0 ;i<NZX;i++){
		  for(int j=0 ;j<NCX;j++){
			  ...
			  fitfun[i][j] = new TF1(buf,MYFUN,-1e3,1e3);
...
//
	...
	float *xposition;
	double middle,peakleft,peakright;
	for(int i=0;i<NZX;i++){
		for(int j=0; j<NCX;j++){
			...
			fitfun[i][j] ->SetParameters(TimeCalib[i][j]->GetMaximum(),peakleft,0.5,TimeCalib[i][j]->GetMaximum(),peakright,0.5);
			TimeCalib[i][j]->Fit(fitfun[i][j],"Q");
			peakV[i][j] = peakV[i][j] + 0.5*(fitfun[i][j] ->GetParameter(1) + fitfun[i][j] ->GetParameter(4) );
			//peakV[i][j] = peakV[i][j]+TimeCalib[i][j]->GetMean();
		}
...
//////****************************************************************//;	
	...
	sprintf(buf,"%s/RingTimeCalib_V1_Iter%d.root",filepath.c_str(),FF);
...
double MYFUN(double *x,double*par){
	double r;
	  r = par[0]*exp(-0.5*(x[0]-par[1])*(x[0]-par[1])/par[2]/par[2])+ par[3]*exp(-0.5*(x[0]-par[4])*(x[0]-par[4])/par[5]/par[5]);
	return r;
}
/*
 void getFiles( string path, vector<string>& files ){  
