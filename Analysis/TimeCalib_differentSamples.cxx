...
//TApplication *a = new TApplication("a", 0, 0);
	  ...
	  int nComP1,nComP2,nComC1,nComC2,nRayP1,nRayP2,nRayC1,nRayC2;
	  float theta,S;
	...
	tree->SetBranchAddress("nComP1",    &nComP1  );
	tree->SetBranchAddress("nComP2",    &nComP2  );
	tree->SetBranchAddress("nRayP1",    &nRayP1  );
	tree->SetBranchAddress("nRayP2",    &nRayP2  );
...
if(FF<Niter-1){
	for(long KK=0;KK<NSamples;KK++){
	//for(long KK=0;KK<1e8;KK++){
		tree->GetEntry(KK);
        if(KK%(int(entries/10.))==0)cout<<"----     "<<(KK/(entries/10))*10<<"%  has been done !!   ----"<<endl;
		//if(nComP1==0||nComP2==0)continue;
		if(abs(S)> int(FOV/2.))continue;
...
//////****************************************************************//;	
	...
	sprintf(buf,"%s/TimeCalib_Samples%d_FOV%d_Iter%d.root",filepath.c_str(),NSamples,int(FOV),FF);
	...
	tr->Write();
    double maximum =  TIMERES->GetBinContent(TIMERES->GetMaximumBin()) ;
    double rightbin = TIMERES->GetBinCenter( TIMERES->GetMaximumBin() );
    double leftbin = -rightbin;
	if(rightbin <0){
		leftbin =rightbin;
        rightbin = -leftbin;
	}
	TF1 *fitfun=new TF1("fitfun","[0]*exp( -0.5*(x-[1])*(x-[1])/[2]/[2] )+[3]*exp(-0.5*(x-[4])*(x-[4])/[5]/[5])+[6]",-5,5);
	fitfun ->SetParameters(maximum, leftbin,0.37,maximum, rightbin,0.37, TIMERES->GetBinContent(1)) ;
	TIMERES ->Fit(fitfun,"Q");
	TIMERES->Write();
	file->Close();
	...
	//for(long KK=0;KK<1e8;KK++){
        ...
        if(KK%(int(entries/10.))==0)cout<<"----     "<<(KK/(entries/10))*10<<"%  has been done !!   ----"<<endl;
		//if(nComP1==0||nComP2==0)continue;
		if(abs(S)> 150 || abs(S)<140)continue;
		if(Adc1<425||Adc1>650||Adc2<425||Adc2>650)continue;
		if(A1<0||A1>=NZX||A2<0||A2>=NZX||T1<0||T2>=NCX||T1<0||T2>=NCX)continue;
