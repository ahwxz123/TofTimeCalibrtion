...
//TApplication *a = new TApplication("a", 0, 0);
	  ...
	  for(int i=0 ;i<NZX;i++){
		  for(int j=0 ;j<NCX;j++){
			  ...
			  fitfun[i][j] = new TF1(buf,MYFUN,-1e3,1e3,6);

			  s[i][j] = new TSpectrum(2,0.42);
		  }
	  }
...
//
	...
	float *xposition;
	double maximum,middle,peakleft,peakright;
	for(int i=0;i<NZX;i++){
		for(int j=0; j<NCX;j++){
			...
			fitfun[i][j] ->SetParameters( maximum,peakleft,0.45,maximum,peakright,0.45);
			TimeCalib[i][j]->Fit(fitfun[i][j],"Q");
			peakV[i][j] = peakV[i][j] + 0.5*(fitfun[i][j] ->GetParameter(1) + fitfun[i][j] ->GetParameter(4) );
			//peakV[i][j] = peakV[i][j]+TimeCalib[i][j]->GetMean();
		}
...
//////****************************************************************//;	
	...
	sprintf(buf,"%s/RingTimeCalib_V1_FOV%d_Iter%d.root",filepath.c_str(),int(FOV),FF);
