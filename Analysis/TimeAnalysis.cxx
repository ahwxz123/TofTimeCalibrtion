...
const unsigned int NPixels=55296;
...
*/
      ...
      int Adc1= -1,Adc2 = -1;
	  double Tdiff= -1.,  Tdiffraw = -1.;
      int A1= -1,  A2= -1,    T1= -1,  T2= -1;
      int co=-1,co1=-1,co2=-1;
      float theta,S;	  

      char TimeFile1[100],TimeFile2[100],TimeFile3[100];
	  sprintf(TimeFile1,"%s/TimeHistgram.ps[",filepath.c_str());
	  sprintf(TimeFile2,"%s/TimeHistgram.ps",filepath.c_str());
	  sprintf(TimeFile3,"%s/TimeHistgram.ps]",filepath.c_str());
	  ...
	  for(int i=0 ;i<NCX;i++){
		  for(int j=0 ;j<NZX;j++){
			  sprintf(buf,"Time_%d_%d",i,j);
			  Time[i][j] = hist1d(buf,100,-5,5,"TimeDiff","Entries");
		  }
	  }
	

	sprintf(buf,"%s/RawCoinwithTimepar.root",filepath.c_str());
	TFile *rootfile =new TFile(buf);
	cout<<buf<<endl;
	TTree *tree;
	tree= (TTree*)rootfile->Get("coin");
...
cout<<entries<<endl;	
	...
	//for(int KK=0;KK<1e6;KK++){
		...
		if(T1<0||T1>=NCX||T2<0||T2>=NCX||A1<0||A1>=NZX||A2<0||A2>=NZX)continue;
		Time[T1][A1]->Fill(-Tdiff );
		Time[T2][A2]->Fill(Tdiff );
	}

	
	sprintf(buf,"%s/Time.root",filepath.c_str());
    ...
    for(int i=0;i<NPixels;i++){
		...
		temp2= i/NCX;
		tp[i%16]->cd(i%16+1);
		Time[temp1][temp2]->Draw();

		if(i%16==15)c1->Print(TimeFile2);
