...
int main(int argc, char *argv[]){
	  ...
	  sprintf(buf,"%s/Timeparameter_CableDelay%dns_sigma300ps_OnePariesModule%dns.root",filepath.c_str(),int(CableDelay),int(CableDelay)-2);
	  ...
	  for(int i=0;i<NsubZ;i++){
	      for(int j=0;j<NsubC;j++){
			 ...
			 if( i>=4&&i<7 &&(j<=1||j==36||j==37)) cc[i][j] = cc[i][j] - 2 ; 
	  ...
	  sprintf(buf,"timeparameter_CableDelay%dns_sigma300ps_OnePariesModule%dns.raw",int(CableDelay),int(CableDelay)-2 );
