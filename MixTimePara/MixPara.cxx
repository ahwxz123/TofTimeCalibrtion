...
int main(int argc, char *argv[]){
	  ...
	  char buf[200];
	  double CableDelay=2; //ns
      string filepath = argv[1];
	  sprintf(buf,"%s/Timeparameter_SubModule.root",filepath.c_str());
	  TFile *file=new TFile(buf,"recreate");
      TTree *tr=new TTree("tr","tr");
	  tr->Branch("timepar",&timepar,"timepar[576][96]/D");

	  TF1 *f1=new TF1("f1","55296*TMath::Gaus(x,0,1)",-10,10);
	  for(int i=0;i<NCX;i++){
	      for(int j=0;j<NZX;j++){
			  //gRandom->SetSeed();
			 timepar[i][j] = CableDelay+f1->GetRandom();
		  }
	  }

	  FILE *fp=fopen("timeparameter1ns.raw","wb");;
