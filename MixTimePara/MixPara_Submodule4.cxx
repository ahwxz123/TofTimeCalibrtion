...
int main(int argc, char *argv[]){
	  ...
	  sprintf(buf,"%s/Timeparameter_CableDelay%dns_sigma300ps_OrderIntervel%dns.root",filepath.c_str(),int(CableDelay),int(CableDelay)-2);
	  ...
	  for(int i=0;i<NsubZ;i++){
	      for(int j=0;j<NsubC;j++){
			 ...
			 if(i%2==0&&j%2==0 || i%2==1&&j%2==1 ) cc[i][j] = cc[i][j] - 2 ; 
	  ...
	  sprintf(buf,"timeparameter_CableDelay%dns_sigma300ps_OrderInterval%dns.raw",int(CableDelay),int(CableDelay)-2 );
